...
import { Menu, X, User, LogOut, MessageSquare, Sun, Moon, Zap, ChevronLeft, AlertTriangle, Briefcase, ListChecks } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16">
            ...
            {user ? (
              <>
                ...
                {profile?.role === 'worker' && (
                  <Link
                    to="/worker/find-jobs"
                    className="flex items-center gap-1.5 text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white px-4 py-2 rounded-xl transition-all duration-300 hover:bg-gray-100 dark:hover:bg-white/5 font-medium"
                  >
              ...
              <>
                <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                  <Link to="/login" className="px-5 py-2.5 bg-white dark:bg-white text-primary-600 font-bold rounded-xl hover:bg-gray-100 transition-all shadow-sm border border-gray-200 dark:border-0">
                    Login
                  </Link>
                </motion.div>
                <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                  <Link to="/signup" className="px-6 py-2.5 bg-gradient-to-r from-primary-600 to-accent-500 text-white font-semibold rounded-xl shadow-lg shadow-primary-500/20 hover:shadow-xl transition-all">
                    Get Started
                  </Link>
                </motion.div>
              </>
          ...
          <div className="md:hidden flex items-center gap-2">
            <motion.button onClick={toggleTheme} className="p-2.5 rounded-xl bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10" whileTap={{ scale: 0.95 }}>
              {theme === 'light' ? <Moon className="w-5 h-5 text-gray-700" /> : <Sun className="w-5 h-5 text-amber-400" />}
            </motion.button>
            {!user && (
              <Link to="/login" className="px-4 py-2 bg-white dark:bg-white text-primary-600 font-bold rounded-xl border border-gray-200 dark:border-0 shadow-sm">
                Login
              </Link>
            )}
            <motion.button
        ...
        {isMenuOpen && (
          ...
          >
            <div className="px-4 py-4 space-y-2">
              {user ? (
                <>
                  <Link to={`/${profile?.role}/dashboard`} className="block px-4 py-3 text-gray-900 dark:text-white rounded-xl hover:bg-gray-100 dark:hover:bg-white/5 transition-colors font-medium" onClick={() => setIsMenuOpen(false)}>
                    Dashboard
                  </Link>

                  {profile?.role === 'worker' && (
                    <Link to="/worker/find-jobs" className="block px-4 py-3 text-gray-900 dark:text-white rounded-xl hover:bg-gray-100 dark:hover:bg-white/5 transition-colors font-medium" onClick={() => setIsMenuOpen(false)}>
                  ...
                  {(profile?.role === 'customer' || profile?.role === 'admin') && (
                    <Link to="/customer/my-jobs" className="block px-4 py-3 text-gray-900 dark:text-white rounded-xl hover:bg-gray-100 dark:hover:bg-white/5 transition-colors font-medium" onClick={() => setIsMenuOpen(false)}>
                  ...
                  {profile?.role === 'worker' && (
                    <Link to="/worker/sos" className="block px-4 py-3 text-red-500 dark:text-red-400 rounded-xl hover:bg-red-50 dark:hover:bg-red-500/10 transition-colors font-medium" onClick={() => setIsMenuOpen(false)}>
                      <AlertTriangle className="w-4 h-4 inline mr-2" />
                      SOS / Safety
                    </Link>
                  )}

                  <Link to="/chat" className="block px-4 py-3 text-gray-900 dark:text-white rounded-xl hover:bg-gray-100 dark:hover:bg-white/5 transition-colors font-medium" onClick={() => setIsMenuOpen(false)}>
                    Messages
                  </Link>
                ...
                <>
                  <Link to="/login" className="block px-4 py-3 text-gray-900 dark:text-white rounded-xl hover:bg-gray-100 dark:hover:bg-white/5 transition-colors font-medium" onClick={() => setIsMenuOpen(false)}>Login</Link>
                  <Link to="/signup" className="block px-4 py-3 bg-gradient-to-r from-primary-600 to-accent-500 text-white rounded-xl text-center font-semibold" onClick={() => setIsMenuOpen(false)}>Sign Up</Link>
                </>
              )}
            </div>
          </motion.div>
