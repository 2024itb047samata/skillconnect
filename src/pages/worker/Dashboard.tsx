...
export default function WorkerDashboard() {
  ...
  const quickActions = [
    ...
    { label: 'Find Jobs', icon: Briefcase, href: '/search', color: 'text-cyan-400' },
  ];
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 relative">
          ...
          >
            <div>
              ...
              <p className="text-surface-400 text-lg">Manage your bookings and grow your business</p>
            </div>
            <motion.button
              ...
              className={`px-5 py-3 rounded-xl font-medium flex items-center gap-2.5 ${
                localStatus === 'available'
                  ? 'bg-emerald-500 shadow-[0_0_30px_rgba(52,211,153,0.4)]'
                  : localStatus === 'busy'
            ...
            {availableJobs.length > 0 ? (
              <div className="space-y-3">
                {availableJobs.slice(0, 4).map((j) => (
                  <div key={j.id} className="p-4 bg-surface-800/50 rounded-xl group hover:bg-surface-800 transition-colors cursor-pointer">
                    ...
                    <div className="flex items-center justify-between mt-3">
                      ...
                      {j.budget_min && (
                        <span className="text-sm font-medium text-gradient">${j.budget_min}</span>
                      )}
                    </div>
                  </div>
                ))}
              ...
              <p className="text-center text-surface-400 py-8">No matching jobs</p>
            )}
          </motion.div>
                    ...
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="flex items-center gap-0.5">
                        {[1, 2, 3, 4, 5].map((s) => (
                          <Star
                            key={s}
                            className={`w-3 h-3 ${s <= r.rating ? 'text-amber-400 fill-amber-400' : 'text-surface-700'}`}
                          />
                        ))}
                      </div>
                    </div>
                    {r.comment && (
                      <p className="text-sm text-surface-400 italic">"{r.comment}"</p>
                    )}
                  </div>
                ))}
              </div>
          ...
          >
            ...
            >
              <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={() => setShowStatusModal(false)} className="p-2 rounded-lg hover:bg-surface-800 transition-colors">
              ...
              <div className="space-y-3">
                {['available', 'busy', 'offline'].map((status) => (
                  <motion.button
                    key={status}
                    onClick={() => setLocalStatus(status as 'available' | 'busy' | 'offline')}
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span className={`w-3 h-3 rounded-full ${
                        ...
                        status === 'busy' ? 'bg-amber-500' : 'bg-surface-600'
                      }`} />
              ...
              <motion.button
                ...
                className="w-full mt-6 py-3 premium-button"
                whileHover={{ scale: 1.01 }}
                whileTap={{ scale: 0.99 }}
              >
                Update Status
          ...
          >
            ...
            >
              <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={() => setShowBookingModal(null)} className="p-2 rounded-lg hover:bg-surface-800 transition-colors">
                  <X className="w-5 h-5 text-surface-400" />
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <motion.button
                  onClick={() => updateBooking(showBookingModal.id, 'rejected')}
                  className="py-3 glass-button rounded-xl font-medium text-surface-300"
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                >
                  Decline
                ...
                <motion.button
                  onClick={() => updateBooking(showBookingModal.id, 'accepted')}
                  className="py-3 premium-button rounded-xl"
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                >
                  Accept
